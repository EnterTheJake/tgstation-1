...
/datum/computer_file/program/contractor_bounties
	filename = "bountyregistry"
	filedesc = "Contractor Bounty Registry"
	extended_desc = "A read-only mirror of a contractor's active bounty board. Targets can be flagged for tracking, 		but extraction pods answer only to the contractor's own uplink."
	program_icon = "tasks"
	size = 6
	program_flags = PROGRAM_UNIQUE_COPY
	undeletable = TRUE
	tgui_id = "ContractorUplink"
	/// Actions relayed to the contractor's uplink. Anything absent, notably call_extraction and buy, is dropped.
	var/static/list/relayed_actions = list("track", "show_mugshots")

/datum/computer_file/program/contractor_bounties/proc/get_contractor()
	var/mob/living/silicon/robot/model/contractor/drone = computer?.loc
	if(!istype(drone))
		return null
	return drone.contractor_ref?.resolve()

/datum/computer_file/program/contractor_bounties/proc/get_uplink()
...
/datum/computer_file/program/contractor_bounties/on_start(mob/living/user)
	. = ..()
	if(!.)
		return
	if(isnull(get_uplink()))
		computer.balloon_alert(user, "no registered contractor!")
		return FALSE

/datum/computer_file/program/contractor_bounties/ui_data(mob/user)
	var/datum/component/uplink/contractor/uplink = get_uplink()
	if(isnull(uplink))
		return list()
	return uplink.ui_data(get_contractor())

/datum/computer_file/program/contractor_bounties/ui_static_data(mob/user)
	var/datum/component/uplink/contractor/uplink = get_uplink()
	if(isnull(uplink))
		return list()
	return uplink.ui_static_data(get_contractor())

/datum/computer_file/program/contractor_bounties/ui_assets(mob/user)
	var/datum/component/uplink/contractor/uplink = get_uplink()
	if(isnull(uplink))
		return ..()
	return uplink.ui_assets(user)

/datum/computer_file/program/contractor_bounties/ui_act(action, list/params, datum/tgui/ui, datum/ui_state/state)
	. = ..()
	if(.)
		return
	if(!(action in relayed_actions))
		return TRUE
	var/datum/component/uplink/contractor/uplink = get_uplink()
	if(isnull(uplink))
		return TRUE
	return uplink.ui_act(action, params, ui, state)
